...
import 'chart/lw_chart_widget.dart';
...
class _StockChartPageState extends State<StockChartPage>
  ...
  Widget _buildIntradayTab(double chartHeight, double timeAxisHeight, double volumeHeight) {
    ...
    // 直接返回 IntradayChart，让 TabBarView 直接传递约束（无需 Column>Expanded 多层包装）
  ...
  Widget _buildKlineTab(double chartHeight, double volumeHeight, double timeAxisHeight) {
    ...
    // On web, use TradingView lightweight-charts for a professional trading UI
    // (candlestick + volume + crosshair + zoom/pan).
    // On other platforms, fall back to the existing ChartViewport.
    if (kIsWeb) {
      final totalHeight = chartHeight + volumeHeight + timeAxisHeight + _klineViewportExtraV;
      return LwChartWidget(
        candles: _kLineDisplayCandles,
        height: totalHeight.clamp(260.0, double.infinity),
        darkMode: true,
        latestCandle: _candlesKLine.isNotEmpty ? _kLineDisplayCandles.last : null,
