...
import 'chart/tv_nav_bar.dart';
import 'chart/tv_ohlc_overlay.dart';
import 'chart/tv_symbol_picker.dart';
import 'chart/tv_timeframe_toolbar.dart';
...
class _GenericChartPageState extends State<GenericChartPage>
  ...
  /// 弹出币种选择器
  void _showSymbolPicker() async {
    final result = await TvSymbolPicker.show(
      context,
      currentSymbol: _effectiveSymbol,
    );
    if (result == null || !mounted) return;
    // 导航到新的币种
    if (MarketRepository.isStockMarket(result.market)) {
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
  ...
  // =========================================================================
  // TradingView-style terminal layout (desktop ≥1180px)
  // =========================================================================

  /// TradingView 风格桌面终端布局
  ///
  /// 结构：TvNavBar(40px) + TvTimeframeToolbar(36px) + Expanded(图表+侧栏)
  Widget _buildTvTerminalLayout({
    required double? price,
    required double? changeVal,
    required double? changePercent,
    required double? prevClose,
    required MarketQuote? q,
  }) {
    final tone =
    ...
    return Column(
      ...
      children: [
        ...
        TvNavBar(
          ...
          onBack: () => Navigator.of(context).maybePop(),
          onPrev: _prevNextIndex > 0 ? _switchToPrev : null,
          onNext: _prevNextIndex >= 0 && _prevNextIndex < _symbolListLength - 1
              ? _switchToNext
              : null,
          marketTypeLabel: _marketTypeLabel(),
          statusLabel: _statusLabel(),
          statusColor: tone,
          onSymbolTap: _showSymbolPicker,
        ),
        // ── 第二层：时间周期工具栏 36px ─────────────────────────────────
        TvTimeframeToolbar(
          ...
          isSidePanelOpen: _isSidePanelOpen,
          onToggleSidePanel: () =>
        ...
        Expanded(
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              // 图表区（自适应剩余空间，LayoutBuilder 获取真实高度）
              Expanded(
                child: ColoredBox(
                  color: ChartTheme.tvBackground,
                  child: LayoutBuilder(
                    builder: (ctx, cc) {
                      final availH = cc.maxHeight;
                      final contentH =
                          (availH - 4).clamp(100.0, double.infinity);
                      final chartH = contentH * _ratioChart;
                      final volH = contentH * _ratioVolume;
                      final timeH = contentH * _ratioTimeAxis;
                      final intradayVolH = contentH * _ratioIntradayVolume;
                        ...
                        content = Center(
                          child: Text(
                            ...
                            style: const TextStyle(
                                color: ChartTheme.textSecondary, fontSize: 13),
                          ),
                        );
                      } else if (_intraday.isEmpty && _daily.isEmpty) {
                        content = _buildEmptyStateCard();
                      } else if (_tabController.index == 0) {
                        content = _intraday.isEmpty
                            ? _buildNoDataHint(true)
                            : IntradayChart(
                                candles: _intraday,
                                prevClose: prevClose,
                                currentPrice: price,
                                chartHeight: contentH *
                                    (_ratioChart + _ratioIntradayVolume * 0.5),
                                timeAxisHeight: timeH,
                                volumeHeight: intradayVolH,
                                periodLabel: '1m',
                                useSessionMarketHours: _isSessionBoundMarket,
                              );
                      } else {
                        content = _daily.isEmpty
                            ? _buildNoDataHint(false)
                            : _buildKlineTab(chartH, volH, timeH);
                      }
                      ...
                      return Stack(
                        children: [
                          ...
                          Positioned(
                            ...
                            child: TvOhlcOverlay(
                              symbol: _effectiveSymbol,
                              periodLabel: _chartTabs[_tabController.index].$1,
                              open: q?.open,
                              high: q?.high,
                              low: q?.low,
                              close: price,
              ...
              // 可折叠侧栏
              if (_isSidePanelOpen) ...[
                Container(width: 1, color: ChartTheme.tvBorder),
                SizedBox(
                  width: ChartTheme.tvSidePanelWidth,
                  child: _buildSidePanel(currentPrice: price, flat: true),
                ),
              ],
            ],
          ),
  ...
  Widget _buildSidePanel({required double? currentPrice, bool flat = false}) {
    final panel = BottomDetailTabs(
      symbol: _effectiveSymbol,
