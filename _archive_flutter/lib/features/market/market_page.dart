...
class _HomeTabState extends State<_HomeTab> with WidgetsBindingObserver {
  ...
  StreamSubscription<WsQuote>? _cryptoWsSub;
  Timer? _cryptoWsFlushTimer;
  Map<String, WsQuote> _pendingCryptoUpdates = {};
  ...
  void initState() {
    ...
    // WebSocket 实时行情：加密货币首页卡片
    MarketWsService.instance.connect();
    MarketWsService.instance.subscribe(_cryptoSymbols.toList());
    _cryptoWsSub = MarketWsService.instance.quoteStream.listen(_onHomeCryptoWsQuote);
  }

  void _onHomeCryptoWsQuote(WsQuote u) {
    if (!mounted) return;
    final sym = u.symbol.trim().toUpperCase();
    final original = _cryptoSymbols.firstWhere(
      (s) => s.toUpperCase() == sym,
      orElse: () => '',
    );
    if (original.isEmpty) return;
    // Buffer the update and flush on a 100ms timer
    _pendingCryptoUpdates[original] = u;
    _cryptoWsFlushTimer ??= Timer(const Duration(milliseconds: 100), _flushHomeCryptoUpdates);
  }

  void _flushHomeCryptoUpdates() {
    _cryptoWsFlushTimer = null;
    if (!mounted) return;
    if (_pendingCryptoUpdates.isEmpty) return;
    final updates = Map<String, WsQuote>.from(_pendingCryptoUpdates);
    _pendingCryptoUpdates.clear();
    setState(() {
      for (final entry in updates.entries) {
        final original = entry.key;
        final u = entry.value;
        final prev = _trendingCryptoQuotes[original];
        if (prev != null && (prev.price - u.price).abs() < 0.0000001) continue;
        ...
        final effectivePct = (prevClose != null && prevClose > 0)
            ...
            : (u.percentChange ?? 0);
        _trendingCryptoQuotes[original] = MarketQuote(
          symbol: original,
          name: prev?.name ?? original,
          price: u.price,
  ...
  void dispose() {
    ...
    _cryptoWsFlushTimer?.cancel();
    _gainersSub?.cancel();
    _losersSub?.cancel();
    _cryptoWsSub?.cancel();
    MarketWsService.instance.unsubscribe(_cryptoSymbols.toList());
    _realtime.dispose();
    super.dispose();
  }
...
class _ForexTabState extends State<_ForexTab> {
  final _market = MarketRepository();
  StreamSubscription<WsQuote>? _realtimeSub;
  ...
  void dispose() {
    _realtimeSub?.cancel();
    if (_realtimeSubscribedSymbols.isNotEmpty) {
      MarketWsService.instance.unsubscribe(_realtimeSubscribedSymbols.toList());
    }
    super.dispose();
  }
  ...
  void _startRealtimeAndSubscribeCurrentPairs() {
    ...
    MarketWsService.instance.subscribe(symbols.toList());
    _realtimeSub = MarketWsService.instance.quoteStream.listen(_onRealtimeQuote);
  }
  ...
  void _onRealtimeQuote(WsQuote u) {
    if (!mounted) return;
...
class _CryptoTabState extends State<_CryptoTab> {
  ...
  static const _realtimeFlushInterval = Duration(milliseconds: 100);
  ...
  final Map<String, WsQuote> _pendingRealtimeUpdates = <String, WsQuote>{};
  Timer? _realtimeFlushTimer;
  Timer? _realtimeResubscribeTimer;
  int _lastRealtimeResubscribeAtMs = 0;

  // 排序缓存：实时更新时只改价格，不重排顺序，避免行位置跳动
  List<(String, String)> _sortOrderCache = [];
  int _sortOrderCacheTab = -1; // 记录缓存对应的 tab，tab 切换时重新排序
  ...
  Future<void> _load() async {
    ...
    _rebuildSortCache(); // 数据加载完成后重建排序缓存
    if (mounted) {
      setState(() => _loading = false);
    }
  }
  ...
  void _startCryptoRealtimeAndSubscribeCurrentPairs() {
    ...
    _realtimeSubscribedSymbols = symbols;
    MarketWsService.instance.subscribe(symbols.toList());
    _realtimeSub = MarketWsService.instance.quoteStream.listen(_onCryptoRealtimeQuote);
  }
  ...
  void _onCryptoRealtimeQuote(WsQuote u) {
    if (!mounted) return;
  ...
  void _flushPendingRealtimeUpdates() {
    ...
    final updates = Map<String, WsQuote>.from(_pendingRealtimeUpdates);
    ...
    // 注意：不在价格更新时调用 _scheduleCryptoRealtimeResubscribe，
    // 避免每 100ms 就取消/重建订阅导致闪烁。
    // 重新订阅只在滚动时（_onListScroll）触发。
  }
  ...
  /// 刷新排序缓存（Tab 切换 / 初始加载时调用）
  void _rebuildSortCache() {
    final list = List<(String, String)>.from(_coins);
    if (_cryptoSubTab == 1) {
      list.sort((a, b) {
        final pa = _quotes[a.$2]?.changePercent ?? double.negativeInfinity;
        final pb = _quotes[b.$2]?.changePercent ?? double.negativeInfinity;
        return pb.compareTo(pa);
      });
    } else if (_cryptoSubTab == 2) {
      list.sort((a, b) {
        final pa = _quotes[a.$2]?.changePercent ?? double.infinity;
        final pb = _quotes[b.$2]?.changePercent ?? double.infinity;
        return pa.compareTo(pb);
      });
    }
    _sortOrderCache = list;
    _sortOrderCacheTab = _cryptoSubTab;
  }

  List<(String, String, MarketQuote?)> _sortedCryptoList() {
    // 缓存失效时才重新排序（tab 切换 or 数据首次加载）
    if (_sortOrderCacheTab != _cryptoSubTab || _sortOrderCache.isEmpty) {
      _rebuildSortCache();
    }
    // 用稳定的排序顺序，搭配最新价格（实时更新只改价格数字，不改行位置）
    return _sortOrderCache.map((e) => (e.$1, e.$2, _quotes[e.$2])).toList();
  }
          ...
          children: [
            _CryptoSubChip(
                ...
                onTap: () {
                  ...
                  setState(() { _cryptoSubTab = 0; _rebuildSortCache(); });
            ...
            _CryptoSubChip(
                ...
                onTap: () {
                  ...
                  setState(() { _cryptoSubTab = 1; _rebuildSortCache(); });
                  WidgetsBinding.instance.addPostFrameCallback(
                      (_) => _scheduleCryptoRealtimeResubscribe(force: true));
                }),
            const SizedBox(width: 8),
            _CryptoSubChip(
                ...
                onTap: () {
                  ...
                  setState(() { _cryptoSubTab = 2; _rebuildSortCache(); });
                  WidgetsBinding.instance.addPostFrameCallback(
                      (_) => _scheduleCryptoRealtimeResubscribe(force: true));
                }),
          ],
