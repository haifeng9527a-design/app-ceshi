-- Add spot_market_value and fix fund distribution display
-- 区分现货市值与合约名义价值，避免合约账户显示误导性「市值」
...
pos as (
  select
    ...
    ), 0)::numeric as contract_notional,
    coalesce(sum(
      case
        when coalesce(product_type, 'spot') in ('perpetual', 'future')
...
agg as (
  select
    ...
    coalesce(pos.spot_market_value, 0)::numeric as spot_market_value,
    coalesce(pos.contract_notional, 0)::numeric as contract_notional,
...
select jsonb_build_object(
  ...
  jsonb_build_object(
    ...
    'market_pct',
      case
        when agg.equity > 0 then (agg.spot_market_value / agg.equity)
        else 0
      end,
    'spot_market_pct',
      case
    ...
    'margin_pct',
      case
        when agg.equity > 0 then (agg.used_margin / agg.equity)
        else 0
      end,
    'frozen_pct',
      case
