-- Extend DB-side trading summary with account dashboard metrics
...
select jsonb_build_object(
  ...
  jsonb_build_object(
    ...
    'cash_available', coalesce((select cash_available from acc), 0),
    ...
    'available_pct',
      case
        when agg.equity > 0 then (coalesce((select cash_available from acc), 0) / agg.equity)
        else 0
      end,
    'market_pct',
      case
